...
| 배치5 검수 | **사용자 진행 대기** (40docs·516행, `gold_review_batch5.xlsx` + 원본 `b-5/`, blind 6: B5-004/010/011/025/026/037) | 학년도 **전수 복귀** 완료(40행), 표본은 학수번호·교수만(18행씩, 표본 12+blind 6). 초안 "; null" 정화 확인(0건). 완료되면 `13_gold_ingest --review data/gold/gold_review_batch5.xlsx --drafts data/gold/drafts_batch5.jsonl --out data/gold/gold_batch5.jsonl` → 게이트 → `16 --batch 5`(지원 완료) → tripwire 판정(학수번호·교수, 배치4 표본 0%가 기준 — 편집 1건이라도 나오면 검토) |
