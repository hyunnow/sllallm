...
| **GwaTop 이식 트랙 (2026-07-13 개시)** | **1단계 완료** | 사용자 결정: ① 1차는 **heuristic+규칙 파이프라인**(인코더는 후속 — 체크포인트가 Colab/Drive에만 있고 EC2 워커 메모리 제약), ② **섀도 비교 후 컷오버**, ③ 기존 OpenAI 파서는 설정 플래그 폴백으로 **영구 보존**(삭제 금지). 단계별 설계·필드 매핑·실행 프롬프트·**진행 체크리스트**는 로컬 `GWATOP_PORT_PLAN.md`(git 제외, 인프라 상세 포함이라 비공개) — **이식 관련 세션은 그 문서를 먼저 읽을 것**(진행 상태의 진실은 그 문서). 0단계: gwatop-backend 태그/브랜치/베이스라인, dev 리베이스, pytest 106 green. 1단계: **이 레포** — 4개 런타임 KB를 `src/syllabus_classifier/config/`로 이동+package-data, `common/config.py` 로딩순서(env→패키지→repo-root), `env.py` 하드코딩 제거, 안정 진입점 `api.py::extract_syllabus`, compiler 이벤트에 `kind` 필드, 신규 테스트 8개, **pytest 156 green**, 깨끗한 venv 설치 스모크 통과. **⚠ 발견: heuristic 경로는 OpenAI 대비 필드 커버리지가 낮다**(샘플에서 class_times 0, 제목·시간 일부 미추출; 정밀도 우선 abstain 특성) — 3단계 섀도가 정량화·컷오버 게이트. **다음 행동: 2단계 통합 — gwatop-backend `feature/sllallm-parser`에서. 커밋+`v0.1.0` 태그 푸시 완료 후 진행** |
