...
| **정책 확정: 파일명은 대학 증거 아님** | 반영 완료 | B5-005/024 판정으로 배치3 시절 doc_id 폴백 철회 (본문·이메일 도메인만). ⚠️ **미결: B2-001/003/008 대학 gold는 구 정책(파일명)으로 채워져 있음** — 새 정책대로면 null로 소급해야 정합. 사용자 답변 대기 |
| **B5-036 정정 미결** | 사용자 미답변 | 학수번호=PHI2007·교수=서상복은 문서에 문자 그대로 존재(대학=서강대는 파일명뿐이라 새 정책상 null이 맞음). 이 셀들이 잔여 rule fab의 전부. 답 주시면 정정→재ingest |
| 배치6 생성 | 대기 | **표본: 규칙대로 학수번호·교수 전수 복귀가 디폴트** (tripwire 편집 5건 중 실질 1건뿐이라 표본 유지도 방어 가능 — 사용자 선택). 초안 증거 게이트 반영됨 — 분노 유발 반복 오류 3종은 배치6 초안에서 사라질 것 |
