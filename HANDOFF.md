...
| **GwaTop 이식 트랙 (2026-07-13 개시)** | 계획 확정 | 사용자 결정: ① 1차는 **heuristic+규칙 파이프라인**(인코더는 후속 — 체크포인트가 Colab/Drive에만 있고 EC2 워커 메모리 제약), ② **섀도 비교 후 컷오버**, ③ 기존 OpenAI 파서는 설정 플래그 폴백으로 **영구 보존**(삭제 금지). 단계별 설계·필드 매핑·실행 프롬프트는 로컬 `GWATOP_PORT_PLAN.md`(git 제외, 인프라 상세 포함이라 비공개) — **이식 관련 세션은 그 문서를 먼저 읽을 것**. 다음 행동: 0단계(gwatop-backend 보존·안전망) 프롬프트 실행 |
