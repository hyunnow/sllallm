...
| **GwaTop 이식 트랙 (2026-07-13 개시)** | **0단계 완료** | 사용자 결정: ① 1차는 **heuristic+규칙 파이프라인**(인코더는 후속 — 체크포인트가 Colab/Drive에만 있고 EC2 워커 메모리 제약), ② **섀도 비교 후 컷오버**, ③ 기존 OpenAI 파서는 설정 플래그 폴백으로 **영구 보존**(삭제 금지). 단계별 설계·필드 매핑·실행 프롬프트·**진행 체크리스트**는 로컬 `GWATOP_PORT_PLAN.md`(git 제외, 인프라 상세 포함이라 비공개) — **이식 관련 세션은 그 문서를 먼저 읽을 것**(진행 상태의 진실은 그 문서). 0단계 결과: gwatop-backend에 태그 `pre-sllallm-openai-parser`·브랜치 `feature/sllallm-parser` 생성, 로컬 dev를 origin/dev로 리베이스(18커밋 뒤처져 있었음), pytest 106 green, OpenAI 파서 baseline 3건 저장. **다음 행동: 1단계(sllallm 패키지화 v0.1.0) — 이 레포에서 진행** |
