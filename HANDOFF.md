...
- Gold: **배치1** 37docs·475셀 확정(게이트: 편집률 38.6%, blind 격차 +9.7%p) / **배치2** 36docs·442셀(학교표기 동치 채점 기준 편집률 30.7%, 격차 +18.0%p ⚠주의 유지) / **배치3** 40docs·560셀 **확정** (편집률 24.55%, blind 격차 +9.78%p — 배치1과 동일 수준, **게이트 통과**; 메모 26건 전량 대응 완료, 커밋 로그 참조).
- 방법 비교(배치3, 실제 표, 잠정): 대학 **98/100/조작0** (이메일 도메인+파일명 폴백 후) · 학기 60/100 (계절 canonical) · 학년도 52/100 · 학점 42/100 · 과목명 65/77 · 학수번호 72/79/조작0 · 총주차 40/81 · 주차별내용 40/38 · 수업시간 22/56/조작11%(잔여 1건=B3-028 gold누락, 우리가 맞음) · 이벤트 하이브리드 cov70%·event-level type 76/129·kind 82/129·완전일치 7/129 (gold가 발표 포함으로 확장돼 분모 커짐) · 무기한과제 조작 93%는 gold 기준 미합의 탓.
- 배치2 재측정(수정 후): 이벤트 하이브리드 완전일치 9→**12**/70 (발표 포함 프롬프트 효과), 대학 94/91/6.
...
| 항목 | 상태 | 다음 행동 |
|---|---|---|
| 배치3 | **완료** (게이트 통과 +9.78%p) | `16 --batch 3` 재측정 완료. 배치4 준비 가능 (`12_gold_review_build` corpus 모드) |
| **gold 기준 합의 3건** | **사용자 결정 대기** | ① 대학 간접추론(이메일·파일명): 배치2는 빈칸 확정, 배치3는 채움 — **배치 간 기준 충돌**, 다음 배치 안내문에 명시 필요. ② B3-028 수업시간: 문서에 "MON WED 10:30-11:45" 명시인데 gold 빈칸(누락 의심) — 정정 여부. ③ 무기한과제 "평가표 안 산출물 포함" 기준(기존 미결). |
...
2. §2의 **gold 기준 합의 3건**을 사용자와 정리 → 합의 내용을 배치4 안내문(검수 시트 '안내' 탭)에 반영.
3. 배치4 생성(`12_gold_review_build` corpus 모드) 또는 §2 비차단 작업(수업시간 커버리지 / 전 코퍼스 레코드 리포트) 진행.
