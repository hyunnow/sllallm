...
| 배치7 검수 | **사용자 진행 대기** (40docs·560행 전수, `gold_review_batch7.xlsx` + 원본 `b-7/`, blind 6: B7-004/010/011/025/026/037) | **동결 재검증 배치** (정책 A/B/C가 안내문에 실린 첫 배치). 완료되면 ingest → 게이트 → `16 --batch 7`(지원 완료) → **신규 관례 ≤1건이면 동결 선언 + 승자 확정(§4-3)**. 미달 시 배치 정책 자체를 사용자와 재논의 |
