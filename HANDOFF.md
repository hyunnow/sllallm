...
| 배치4 검수 | **사용자 진행 대기** (37docs·458행 생성 완료, `gold_review_batch4.xlsx` + 원본 `b-4/`) | **표본 검수 첫 적용**: 학수번호·교수·학년도는 비-blind 표본 11docs+blind 6docs만 행 존재(60행 절감). 완료되면 `13_gold_ingest --review data/gold/gold_review_batch4.xlsx --drafts data/gold/drafts_batch4.jsonl --out data/gold/gold_batch4.jsonl` → 게이트 보고 → `16 --batch 4` (지원 완료). **tripwire**: 표본 필드 편집률이 배치3 전수(0%·6%·10%)의 2배 초과 시 배치5는 전수 복귀 |
...
2. 배치4 검수 완료 여부 확인 → 완료면 §2 표의 게이트 절차 (ingest → 게이트 보고 → `16 --batch 4`; **표본 필드 tripwire 판정 포함**).
3. 미완이면 §2 비차단 작업(수업시간 커버리지 / 전 코퍼스 레코드 리포트 / 중복 실라버스 탐지) 진행.
