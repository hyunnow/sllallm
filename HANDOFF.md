...
| 배치6 검수 | **사용자 진행 대기** (40docs·560행 **전수**, `gold_review_batch6.xlsx` + 원본 `b-6/`, blind 6: B6-004/010/011/025/026/037) | **규칙 동결 검증 배치** — 표본 없음(신규 엣지 탐지 우선). 초안 게이트 위반 0건 사전 확인(환각 연도·교시코드·null 잔재). 완료되면 ingest → 게이트 → `16 --batch 6`(지원 완료) → **신규 관례 ≤1건이면 규칙 동결 선언 + 승자 확정 절차(§4-3 dev/holdout)** |
