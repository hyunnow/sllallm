...
NOTATION = (
    ...
    "  발표(presentation)·퀴즈 등 일정성 산출물도 이벤트(타입 other) — B3-003.\n"
    "  날짜 없는 시험은 이벤트에 `제목 | exam | null | uncertain` 로 (무기한과제에 중복 금지).\n"
    "무기한과제: 날짜 참조가 전혀 없는 과제류 제목만 ` ; `로 (평가표 안 산출물 포함,\n"
    "  이벤트와 중복 기재 금지 — 2026-07-10 통일 규칙).\n"
    "주차별내용: `Week N: 내용` 을 ` ; `로 (내용은 원문 언어).\n"
    "  날짜/세션 기반 문서는 주 단위로 묶어 `Week N (날짜): 내용1 / 내용2` (B6-019/020/026 확정).\n"
    "  '주별학습목표'와 '강의내용'이 둘 다 있으면 강의내용만, 없으면 목표로 폴백 (B6-025 확정).\n"
    "  주차/날짜 구조가 아예 없는 토픽형 실라버스는 topic을 ` ; `로 나열 (Week 접두 없이 — B7-008).\n"
    "  주 1회 수업의 N차시/N회차 = N주차 (B7-040 확정).\n"
    "  총주차는 날짜가 명시된 시험기간까지 포함해 계산 (B6-019 확정).\n"
    "과제 날짜 의미: 이벤트 날짜는 행동일 — 마감 명시 시 마감일, 출제일만 있으면 그대로 raw\n"
    "  (마감 추정 금지). 매일/매주 반복 과제는 `제목 | assignment | 기간 | recurring` 이벤트로\n"
    "  (무기한과제 아님 — 무기한과제는 날짜 참조가 전혀 없는 것만; B6-026 확정).\n"
    "연락처: `email ; phone` (있는 것만, 이메일 우선).\n"
    "대학=학교(학과 아님) — 본문 표기 > 이메일 도메인 > 파일명(kocw류 아카이브 명명)\n"
    "  순으로 간접추론 허용, 셋 다 없으면 빈칸 (2026-07-12 최종 확정).\n"
    "연락처: 종류 라벨 붙이지 않음 — 값만 기재 (2026-07-12 확정).\n"
